...
- 기대효과
  -  청각장애인들과 일반인 사이의 커뮤니케이션을 도와주는 용도로 일반인들이 수어를 알 수 있도록 도와준다.
- 테스트 시나리오
  1. 청각장애인이 수어를 사용하는 경우
  2. 상대방(일반인)이 카메라로 대화 대상의 수어를 촬영.
  3. 촬영한 영상이 서버로 전송 -> 서버에서 수어 인식을 통해 번역 작동
  4. 번역된 문장이 화면에 출력. (핸드폰 화면 or 스마트 글래스)
...
- 빅데이터
  -  수어 영상 데이터셋 확보
-  AI
  - 수어 영상을 인식하는 기능. 인식한 단어를 텍스트로 변환하여 번역기에 전달 -> 번역기가 어순이 다른 수어를한글 문장으로 번역.  
-  IOT
  - 파이 카메라를 이용해 수어 영상을 서버로 전달(mqtt 통신 이용) 또는 안드로이드 카메라를 통해 수어 영상을 서버로 전달
  -  AI 모델을 통해 나온 한글 텍스트를 oled로 띄우기
- 클라우드
  - AWS S3 클라우드 서비스 구축
  - 클라우드 서비스에 mqtt 설치
  - ai모델이 분석후 단어를 mqtt로 통해 전달
...
| ai | iot | 빅데이터 | 클라우드 |
| --- | --- | --- | --- |
| 상품인식 object detection | 버튼(푸쉬 버튼)을 누르면 사용자가 어디에 있는지 알려주는 기 | 데이터 크롤링 | mqtt 설치 |
|  | 음성으로 매장 위치 알려주는것   | 데이터 분석(통계)  matplotlib | aws s3 설치 |
|  | 버튼을 누르면 웹에서 호출  | 제품 추천 | aws rdbs 설치 |
|  | 상품 위치를 dwd 1000 모듈을 이용해  품목 위치 확인https://smartstore.naver.com/jy-soft/products/5563264345 |  |  |
|  |   |  |  |
|  | 버튼을 누르면 웹에서 호출  |  |  |
|  | 안드로이드 |  |  |
|  | 상품 인식() - 상품 정보 전체 |  |  |
